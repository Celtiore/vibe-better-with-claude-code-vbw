...
7. **Existing release branch:** Check local and remote for existing `release/v*` branches:
   ...
   - Otherwise (non-dry-run), initialize counters: `{cleaned}=0`, `{failed}=0`, `{remote-skipped}=0`. For each `release/v{version}` branch:
     ...
     - **No-push gating:** If `--no-push`, do **not** run `git push origin --delete ...`. Increment `{remote-skipped}` and display: "○ --no-push: skipping remote deletion for `release/v{version}` (remote branch/PR may still exist)."
   ...
   - If `{failed} > 0` and `{cleaned} == 0` and `{remote-skipped} == 0` → STOP: "All branch deletions failed. Check permissions and retry."
