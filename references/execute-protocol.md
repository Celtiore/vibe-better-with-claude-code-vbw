...
1. Send `shutdown_request` via SendMessage to EVERY active teammate (excluding yourself — the orchestrator controls the sequence, not the lead agent) — do not skip any. The SendMessage JSON body must include at minimum: `{"type": "shutdown_request", "reason": "phase_complete", "team_name": "vbw-phase-{NN}"}` (this is a simplified form — the full V2 envelope nests these under `payload`, but agents are instructed to match on `"type":"shutdown_request"` regardless of structure). Teammates respond by calling SendMessage with `type: "shutdown_response"`.
