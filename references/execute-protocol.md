...
The plan_path argument is passed for context. compile-context.sh reads installed skill names from STATE.md and emits a lightweight "Installed Skills" directive listing skill names only. The Dev agent calls `Skill(skill-name)` for each relevant skill, leveraging Claude Code's native progressive disclosure for on-demand loading.
