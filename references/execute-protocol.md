...
The plan_path argument is passed for context. Skill activation uses a three-layer architecture:
- **Layer 1 — Agent Markdown (baseline):** `vbw-dev.md` and `vbw-lead.md` contain unconditional skill evaluation instructions. No escape hatches — agents must always evaluate skills before implementing.
- **Layer 2 — UserPromptSubmit hook (primary for teammates):** `skill-eval-prompt-gate.sh` fires on the first prompt per session (PID-scoped marker prevents re-injection). This covers agent team teammates, which are separate Claude Code sessions where SubagentStart does NOT fire. The hook calls `evaluate-skills.sh` to get installed skill names and injects a compact EVALUATE/ACTIVATE/IMPLEMENT gate (~100 tokens) into `additionalContext`.
- **Layer 3 — SubagentStart hook (supplementary for direct subagents):** `skill-evaluation-gate.sh` fires for non-team subagent spawns (e.g., `/vbw:fix` direct Agent tool calls). Injects a full MANDATORY SKILL EVALUATION SEQUENCE with skills table into `additionalContext` at spawn-time (high attention position). The hook calls `evaluate-skills.sh` to read installed skill names from STATE.md's `**Installed:**` line, locate each skill's SKILL.md on disk (3 search paths: `$CLAUDE_DIR/skills/`, `.claude/skills/`, `~/.agents/skills/`), and extract the `description` field from YAML frontmatter.
