...
case "$AGENT_NAME" in
  *scout*)
    ...
    ;;
  *dev*)
    PRIORITIES="Preserve commit hashes, file paths modified, deviation decisions, current task number. After compaction, if $PLANNING_DIR/codebase/META.md exists, re-read CONVENTIONS.md, PATTERNS.md, STRUCTURE.md, and DEPENDENCIES.md (whichever exist) from $PLANNING_DIR/codebase/. Read muninndb_vault from .vbw-planning/config.json then call muninn_guide(vault) for vault-aware instructions, then muninn_activate(vault, context: your current task context) to recover cognitive memory (decisions, patterns, conventions from prior phases)"
    ...
    ;;
  *qa*)
    PRIORITIES="Preserve pass/fail status, gap descriptions, verification results. After compaction, if $PLANNING_DIR/codebase/META.md exists, re-read TESTING.md, CONCERNS.md, and ARCHITECTURE.md (whichever exist) from $PLANNING_DIR/codebase/. Read muninndb_vault from .vbw-planning/config.json then call muninn_guide(vault) for vault-aware instructions, then muninn_activate(vault, context: your verification context) to recover cognitive memory"
    ;;
  *lead*)
    PRIORITIES="Preserve phase status, plan structure, coordination decisions. After compaction, if $PLANNING_DIR/codebase/META.md exists, re-read ARCHITECTURE.md, CONCERNS.md, and STRUCTURE.md (whichever exist) from $PLANNING_DIR/codebase/. Read muninndb_vault from .vbw-planning/config.json then call muninn_guide(vault) for vault-aware instructions, then muninn_activate(vault, context: your phase goal) to recover prior decisions and planning context"
    ;;
  *architect*)
    PRIORITIES="Preserve requirement IDs, phase structure, success criteria, key decisions. After compaction, if $PLANNING_DIR/codebase/META.md exists, re-read ARCHITECTURE.md and STACK.md (whichever exist) from $PLANNING_DIR/codebase/. Read muninndb_vault from .vbw-planning/config.json then call muninn_guide(vault) for vault-aware instructions, then muninn_activate(vault, context: your scoping context) to recover prior architectural decisions"
    ;;
  *debugger*)
    PRIORITIES="Preserve reproduction steps, hypotheses, evidence gathered, diagnosis. After compaction, if $PLANNING_DIR/codebase/META.md exists, re-read ARCHITECTURE.md, CONCERNS.md, PATTERNS.md, and DEPENDENCIES.md (whichever exist) from $PLANNING_DIR/codebase/. Read muninndb_vault from .vbw-planning/config.json then call muninn_guide(vault) for vault-aware instructions, then muninn_activate(vault, context: your bug description) to check for similar past issues"
    ;;
  *docs*)
    PRIORITIES="Preserve doc structure, naming conventions, style decisions. After compaction, if $PLANNING_DIR/codebase/META.md exists, re-read CONVENTIONS.md, PATTERNS.md, and STRUCTURE.md (whichever exist) from $PLANNING_DIR/codebase/. Read muninndb_vault from .vbw-planning/config.json then call muninn_guide(vault) for vault-aware instructions, then muninn_activate(vault, context: your documentation task context) to recover prior doc patterns and conventions"
    ;;
  *)
